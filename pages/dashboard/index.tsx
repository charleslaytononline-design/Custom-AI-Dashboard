...
export default function Dashboard() {
  ...
  async function loadPages(userId: string) {
    ...
    else createPage('My Dashboard', userId)
  }
  ...
  async function createPage(name: string, userId?: string) {
    ...
    const { data, error } = await supabase.from('pages').insert({ user_id: uid, name, code: STARTER_CODE }).select().single()
    if (!error && data) { setPages(prev => [data, ...prev]); setActivePage(data); setMessages([]); setShowNewPage(false); setNewPageName(''); setSidebarTab('chat') }
  }
          ...
          {sidebarTab === 'chat' && (
            <>
              <div style={s.msgs}>
                {messages.length === 0 && (
                  <div style={s.empty}>
                    ...
                    <div style={s.chips}>
                      {['Admin dashboard with sidebar','Inventory tracker with table','Sales dashboard with charts','User management panel'].map(t => (
                ...
                {messages.map((msg, i) => (
                  <div key={i} style={{ ...s.msgRow, ...(msg.role==='user' ? s.msgRight : {}) }}>
                    <div style={{ ...s.bubble, ...(msg.role==='user' ? s.bubbleUser : msg.isPlan ? s.bubblePlan : s.bubbleAI) }}>
                      {msg.isPlan && <div style={s.planLabel}>📋 Plan — review and approve</div>}
              ...
              <div style={s.inputArea}>
                <div style={s.modeRow}>
                  <button onClick={() => { setMode('plan'); setPendingPlan(null) }} style={{ ...s.modeBtn, ...(mode==='plan' ? s.modeBtnOn : {}) }}>
                    📋 Plan
                  </button>
                  <button onClick={() => setMode('build')} style={{ ...s.modeBtn, ...(mode==='build' ? s.modeBtnOn : {}) }}>
                    ⚡ Build
                  </button>
                  <span style={{ flex:1 }} />
...
const s: Record<string, React.CSSProperties> = {
  ...
  codeBtnOn: { background:'var(--accent-10)',borderColor:'var(--accent-30)',color:'var(--accent)' },
  ...
  bubblePlan: { background:'var(--accent-7)',border:'1px solid var(--accent-20)',color:'var(--text-2)',maxWidth:'100%',width:'100%' },
  ...
  errBox: { background:'var(--danger-12)',border:'1px solid var(--danger-25)',borderRadius:8,padding:'8px 12px',fontSize:12,color:'var(--danger)' },
  ...
  modeBtnOn: { background:'var(--accent-15)',borderColor:'var(--accent-30)',color:'var(--accent)' },
  ...
  pageItemOn: { background:'var(--accent-7)' },
