...
export default function ProjectBuilder() {
  ...
  async function loadProject() {
    ...
    if (!proj) { router.push('/home'); return }
    setProject(proj)
    const { data: pgs } = await supabase.from('pages').select('*').eq('project_id', projectId).order('created_at')
    if (pgs && pgs.length > 0) { setPages(pgs); setActivePage(pgs[0]) }
    loadTokens()
  }

  async function loadTokens() {
    const { data } = await supabase.from('usage').select('tokens').eq('user_id', user?.id)
    if (data) setTotalTokens(data.reduce((s:number,r:any)=>s+r.tokens,0))
  }

  async function createPage(name: string) {
    if (!name.trim()) return
    const code = getStarterCode()
    const { data } = await supabase.from('pages').insert({ project_id: projectId, user_id: user.id, name: name.trim(), code }).select().single()
    if (data) { setPages(prev=>[...prev,data]); setActivePage(data); setMessages([]); setShowNewPage(false); setNewPageName(''); setSidebarTab('chat') }
  }

  async function savePage(code: string) {
    if (!activePage) return
    const now = new Date().toISOString()
    const { data } = await supabase.from('pages').update({ code, updated_at: now }).eq('id', activePage.id).select().single()
    if (data) { setActivePage(data); setPages(prev=>prev.map(p=>p.id===data.id?data:p)); renderIframe(data.code) }
  ...
  async function callAPI(msgs: any[], planOnly = false) {
    const res = await fetch('/api/claude', {
      ...
      body: JSON.stringify({ messages: msgs, pageCode: activePage?.code, pageName: activePage?.name, allPages: pages, planOnly }),
    })
  ...
  async function trackUsage(tokens: number) {
    ...
    await supabase.from('usage').insert({ user_id: user.id, project_id: projectId, page_id: activePage?.id, tokens })
  ...
  async function getPlan() {
    ...
    try {
      ...
      await trackUsage(data.tokensUsed)
    ...
    <div style={s.root}>
      <div style={s.topbar}>
        ...
        <div style={s.topRight}>
          {activePage && <button onClick={()=>setShowCode(!showCode)} style={{...s.codeBtn,...(showCode?s.codeBtnOn:{})}}>{showCode?'Hide Code':'</> View Code'}</button>}
          <div style={s.tokenBadge}><span style={{color:'#444',fontSize:10}}>tokens</span><span style={{color:'#7c6ef7',fontWeight:600,fontSize:12}}>{totalTokens.toLocaleString()}</span></div>
      ...
      <div style={s.main}>
          ...
          {sidebarTab==='chat' && (
            <>
              <div style={s.msgs}>
                {messages.length===0 && (
                  <div style={s.empty}>
                    ...
                    <p style={{color:'#555',fontSize:13,textAlign:'center',lineHeight:1.6,marginBottom:20}}>
                      {mode==='plan'?'Describe what you want — I\'ll plan it first.':'Describe what to build and I\'ll create it instantly.'}
                    </p>
                    <div style={s.chips}>
                ...
                {loading&&<div style={s.msgRow}><div style={{...s.bubble,...s.bubbleAI}}><div style={{display:'flex',gap:5}}>{[0,1,2].map(i=><span key={i} style={{...s.dot,animationDelay:`${i*0.2}s`}}/>)}<span style={{color:'#555',fontSize:12,marginLeft:4}}>{mode==='plan'?'Planning...':'Building...'}</span></div></div></div>}
              ...
              <div style={s.inputArea}>
                <div style={s.modeRow}>
                  ...
                  <span style={{flex:1}}/>
                  <span style={{fontSize:11,color:'#444'}}>{mode==='plan'?'Plan first':'Direct build'}</span>
                </div>
                <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendMessage()}}} placeholder={mode==='plan'?'Describe what to plan...':'Describe what to build...'} rows={3} style={s.textarea} disabled={loading}/>
                <button onClick={sendMessage} disabled={loading||!input.trim()} style={s.sendBtn}>{loading?'Working...':mode==='plan'?'📋 Create Plan':'⚡ Build'}</button>
              </div>
            </>
          )}
              ...
              <div style={{padding:12}}>
                {showNewPage?(
                  <div style={{display:'flex',gap:6}}>
                    <input autoFocus value={newPageName} onChange={e=>setNewPageName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')createPage(newPageName)}} placeholder="Page name..." style={s.newInput}/>
        ...
        <div style={s.right}>
          <div style={s.previewBar}>
            <div style={s.urlBar}>{project.name} › {activePage?.name||'No page'}</div>
...
<body class="bg-[#0a0a0a] min-h-screen flex items-center justify-center p-10">
  <div class="text-center max-w-lg">
    ...
    <p class="text-white/50 text-sm leading-relaxed mb-8">Use the AI chat on the left to build anything.</p>
    <div class="bg-brand/10 border border-brand/20 rounded-xl p-4 text-brand text-sm">Try: "Build an admin dashboard with sidebar and stats"</div>
  </div>
</body>
</html>`
}
...
const s: Record<string,React.CSSProperties> = {
  ...
  backBtn:{padding:'4px 10px',background:'none',border:'1px solid rgba(255,255,255,0.08)',borderRadius:6,color:'#888',fontSize:12,cursor:'pointer'},
  sep:{color:'#333'},
  projectName:{fontSize:13,fontWeight:500,color:'#f0f0f0'},
  pageName:{fontSize:13,color:'#555'},
  topRight:{display:'flex',alignItems:'center',gap:12},
  codeBtn:{padding:'5px 12px',background:'none',border:'1px solid rgba(255,255,255,0.1)',borderRadius:7,color:'#666',fontSize:12,cursor:'pointer'},
  ...
  signOutBtn:{padding:'5px 11px',background:'none',border:'1px solid rgba(255,255,255,0.08)',borderRadius:6,color:'#555',fontSize:12,cursor:'pointer'},
