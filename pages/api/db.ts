...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (action === 'insert') {
      ...
      // Audit log: insert
      appDb.from('platform_logs').insert({ event_type: 'db_mutation', severity: 'info', message: `insert on ${table}`, metadata: { sourceFile: 'pages/api/db.ts', userId: sessionUserId, projectId, table, action: 'insert' } }).then(() => {}, () => {})
      return res.status(200).json({ success: true, data: result })
    }
    ...
    if (action === 'update') {
      ...
      if (error) return res.status(200).json({ success: false, error: sanitizeError(error) })
      // Audit log: update
      appDb.from('platform_logs').insert({ event_type: 'db_mutation', severity: 'info', message: `update on ${table}`, metadata: { sourceFile: 'pages/api/db.ts', userId: sessionUserId, projectId, table, action: 'update', rowId: id } }).then(() => {}, () => {})
      return res.status(200).json({ success: true, data: result })
    }
    ...
    if (action === 'delete') {
      ...
      if (error) return res.status(200).json({ success: false, error: sanitizeError(error) })
      // Audit log: delete
      appDb.from('platform_logs').insert({ event_type: 'db_mutation', severity: 'info', message: `delete on ${table}`, metadata: { sourceFile: 'pages/api/db.ts', userId: sessionUserId, projectId, table, action: 'delete', rowId: data.id } }).then(() => {}, () => {})
