...
export default function DiffViewer({ changes, onClose }: DiffViewerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div
        className="bg-[var(--bg-2)] border border-white/10 rounded-xl w-full max-w-[960px] h-[80vh] flex flex-col overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/[0.07] shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-[var(--text)]">Changes</span>
            <span className="text-[10px] text-[var(--text-3)] px-1.5 py-0.5 bg-white/5 rounded">{changes.length} file{changes.length !== 1 ? 's' : ''}</span>
          </div>
          <button
            onClick={onClose}
            className="px-2 py-1 bg-transparent border border-[var(--border)] rounded-md text-[var(--text-2)] text-xs cursor-pointer hover:text-white"
          >
            ✕ Close
          </button>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* File list sidebar */}
          <div className="w-[200px] border-r border-[var(--border)] overflow-y-auto shrink-0 bg-[#0c0c0c]">
            {changes.map((change, i) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 border-none cursor-pointer flex items-center gap-2 text-xs ${
                  i === selectedIndex
                    ? 'bg-brand/10 text-[var(--text)]'
                    : 'bg-transparent text-[var(--text-2)] hover:bg-white/5'
                }`}
              >
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-1.5 border-b border-white/[0.05] bg-[var(--bg)] shrink-0">
              <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${actionColors[selected.action]}`}>
                {selected.action}
              </span>
              <span className="text-xs text-[var(--text-2)] font-mono">{selected.path}</span>
            </div>
