  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between px-3 py-1.5">
        <span className="text-[10px] text-[#555] font-semibold uppercase tracking-wider">Env Variables</span>
        <button
      ...
      {showAdd && (
        <div className="px-3 pb-2 flex flex-col gap-1.5">
          <input
            ...
            className="px-2 py-1.5 bg-[#0a0a0a] border border-white/10 rounded text-xs text-white outline-none focus:border-brand/50 font-mono uppercase"
          />
          <input
            value={newValue}
            ...
            className="px-2 py-1.5 bg-[#0a0a0a] border border-white/10 rounded text-xs text-white outline-none focus:border-brand/50 font-mono"
          ...
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-1.5 cursor-pointer">
              ...
              />
              <span className="text-[10px] text-[#888]">Secret (hidden in UI)</span>
            </label>
            <button
              onClick={addVar}
      ...
      <div className="px-3 space-y-0.5">
        {vars.length === 0 ? (
          <div className="text-[10px] text-[#444] py-1">No environment variables set</div>
        ) : (
          vars.map(v => (
            <div key={v.id} className="flex items-center justify-between group py-0.5">
              <div className="flex items-center gap-1.5 min-w-0 flex-1">
                <span className="text-[10px] text-[#aaa] font-mono truncate">{v.key}</span>
                <span className="text-[9px] text-[#444] font-mono truncate max-w-[80px]">
                  {v.is_secret ? '••••••' : v.value}
                </span>
              </div>
              <button
                onClick={() => deleteVar(v.id)}
                className="text-[10px] text-[#555] hover:text-red-400 bg-transparent border-none cursor-pointer opacity-0 group-hover:opacity-100 shrink-0"
              >
                ✕
              </button>
            </div>
