...
export default function DatabaseChoiceModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div
        className="bg-[var(--bg-2)] border border-white/10 rounded-2xl p-6 w-full max-w-[520px] flex flex-col gap-5"
      >
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-brand/15 flex items-center justify-center text-xl">🗄</div>
          <div>
            <h2 className="text-base font-semibold text-[var(--text)]">Your app needs a database</h2>
            <p className="text-[var(--text-2)] text-[13px] mt-0.5">Choose where to store your app's data</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            disabled={saving}
            className="w-full text-left p-4 bg-[var(--bg)] border border-[var(--border)] rounded-xl hover:border-brand/30 hover:bg-brand/[0.03] transition-all cursor-pointer disabled:opacity-50 group"
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="text-sm font-medium text-[var(--text)] group-hover:text-brand transition-colors">
                ...
                <div className="flex gap-3 mt-2">
                  <span className="text-[10px] text-[var(--text-3)]">✓ Instant setup</span>
                  <span className="text-[10px] text-[var(--text-3)]">✓ Fully managed</span>
                  <span className="text-[10px] text-[var(--text-3)]">✓ Secure & isolated</span>
                </div>
              </div>
            </div>
          </button>
          ...
          <button
            ...
            disabled={saving}
            className="w-full text-left p-4 bg-[var(--bg)] border border-[var(--border)] rounded-xl hover:border-white/20 hover:bg-white/[0.02] transition-all cursor-pointer disabled:opacity-50 group"
          >
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-[var(--text-2)] text-sm shrink-0 mt-0.5">🔗</div>
              <div className="flex-1">
                <div className="text-sm font-medium text-[var(--text-2)] group-hover:text-white transition-colors">
                ...
                <div className="flex gap-3 mt-2">
                  <span className="text-[10px] text-[var(--text-3)]">✓ Full control</span>
                  <span className="text-[10px] text-[var(--text-3)]">✓ Own auth</span>
                  <span className="text-[10px] text-[var(--text-3)]">✓ Requires setup</span>
                </div>
              </div>
            </div>
          </button>
        ...
        <p className="text-[11px] text-[var(--text-3)] text-center">You must choose an option to continue building</p>
      </div>
    </div>
  )
}
